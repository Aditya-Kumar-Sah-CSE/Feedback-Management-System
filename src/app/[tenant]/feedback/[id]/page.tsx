    ...
    <div className="min-h-screen flex flex-col bg-slate-50 text-slate-800">
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full px-2.5 sm:px-6 lg:px-8 py-3.5 sm:py-10 space-y-4 sm:space-y-6">
        <div>
          <Link
            href={`/${tenant.slug}/feedback`}
            className="inline-flex items-center gap-1.5 text-xs font-semibold text-slate-500 hover:text-blue-600 mb-2.5 transition-colors"
          >
        ...
        {!result.success || !result.form ? (
          <div className="bg-white rounded-2xl p-5 sm:p-10 border border-slate-200 text-center space-y-3 sm:space-y-4 shadow-xs">
            <div className="w-12 h-12 rounded-full bg-red-50 text-red-600 mx-auto flex items-center justify-center">
              <AlertCircle className="w-6 h-6" />
            </div>
            <div className="space-y-1">
              <h3 className="text-sm sm:text-base font-bold text-slate-900">Feedback Form Unavailable</h3>
            ...
            <div className="pt-1 sm:pt-2">
              <Link
                href={`/${tenant.slug}/feedback`}
                className="inline-flex items-center gap-2 px-4 sm:px-5 py-2 sm:py-2.5 bg-blue-600 hover:bg-slate-900 text-white rounded-xl text-xs font-bold transition-colors"
              >
        ...
        <div className="bg-white rounded-2xl p-3.5 sm:p-6 border border-slate-200 shadow-xs flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4 mt-4 sm:mt-6">
          <div className="flex items-center gap-3 text-center sm:text-left">
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-white border border-slate-200 flex items-center justify-center shrink-0 overflow-hidden p-1 shadow-xs">
              {tenant.logo ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img
                  src={tenant.logo}
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                Explore all active feedback forms available for {tenant.name}.
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2 w-full sm:w-auto shrink-0">
            <Link
              href={`/${tenant.slug}/feedback`}
              className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-4 sm:px-5 py-2 sm:py-2.5 bg-slate-900 hover:bg-blue-600 text-white rounded-xl text-xs font-bold transition-colors shadow-xs"
            >
      ...
      <footer className="bg-slate-900 text-slate-400 text-xs py-6 px-4 border-t border-slate-800 mt-auto">
        <div className="max-w-4xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-3 text-center sm:text-left">
          ...
          <div className="flex items-center gap-3">
            <Link href={`/${tenant.slug}/feedback`} className="text-slate-300 hover:text-white transition-colors">
              More Forms
            </Link>
